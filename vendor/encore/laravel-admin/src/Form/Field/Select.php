...
EOT;
     ...
     *
     */
    public function hideField($className)
    {
        $fiedlClassName = '.' . trim($className, '.');
...
$(function(){
    $(document).on('change',"{$this->getElementClassSelector()}",function(){
        ...
        var _val = _this.val();
        if(_val == 2){
            $("{$fiedlClassName}").parents('.form-group').hide();
            $("{$fiedlClassName}").attr('disable',true);
        }
        if(_val == 1){
            $("{$fiedlClassName}").parents('.form-group').show();
            $("{$fiedlClassName}").attr('disable',false);
        }
    });
...
EOT;
    ...
    /**
     * 根据select值隐藏相应表单内容，针对hasmany表单
     *
     */
    public function hideField1($className)
    {
        $fiedlClassName = trim($className, '.');
...
$(function(){
    $(document).on('change',"{$this->getElementClassSelector()}",function(){
        ...
        var fieldclassname = name.substr(0, name.lastIndexOf("["))+'[' + "{$fiedlClassName}" + ']';
        var find = "select[name='" + fieldclassname + "']";
        console.log(fieldclassname);
        if(_val == 2){
            $(find).parents('.form-group').hide();
            $(find).attr('disable',true);
        }
        if(_val == 1){
            $(find).parents('.form-group').show();
            $(find).attr('disable',false);
        }
    });
    $("{$this->getElementClassSelector()}").trigger('change');
})
